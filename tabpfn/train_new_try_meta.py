...
def train(lr=0.0001, wandb_name='', num_augmented_datasets=0, epochs = 100, weight_decay=0.0, augmentation_config = []):
    ...
    scheduler = get_cosine_schedule_with_warmup(optimizer, warmup_epochs, epochs*50 if epochs is not None else 100) # when training for fixed time lr schedule takes 100 steps
