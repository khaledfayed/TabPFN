...
from meta_dataset_loader import load_OHE_dataset, meta_dataset_loader3, shuffle_dataset_features
import numpy as np
...
def train(lr=0.00001, wandb_name='', num_augmented_datasets=0, epochs = 100, weight_decay=0.0):
    ...
    datasets2 = [{'data':datasets1[0]['data'][:1024], 'target': datasets1[0]['target'][:1024], 'id': 0}]
    x_test = datasets1[0]['data'][1024:]
    y_test = datasets1[0]['target'][1024:]
    
    datasets = [{'data':shuffle_dataset_features(datasets2[0]['data']), 'target': datasets2[0]['target'], 'id': 0 } for i in range(num_augmented_datasets)]
    #training setup
    ...
    for batch in range(epochs):
        ...
        for i in range(len(support_dataset)):
            ...
            X_full_test = np.concatenate([support_dataset[i]['x'], x_test], axis=0)
            ...
            acc = accuracy_score( torch.from_numpy(query_dataset[i]['y']).long().flatten().cpu(), torch.argmax(output.reshape(-1, num_classes).detach().cpu(), axis=1) )
            loss.backward()
